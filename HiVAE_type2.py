...
class HiVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        phi_1.mu_p.data, phi_1.logvar_p.data = self.encoder1(x)

        optim_infer = torch.optim.Adam(phi_1.parameters(), lr=1e-2)
        inference_params = list(phi_1.parameters())

        grad_switch(inference_params, grad_bool=True)
        for infer_iter in range(self.n_iters):
            optim_infer.zero_grad()
            _, _, _, total_loss = self.iter_inference(x, phi_1, phi_2=None)
            optim_infer.step()
        ...
        z1_sample, _ = self.r_sampling(phi_1.mu_p, phi_1.logvar_p)

        phi_2.mu_p.data, phi_2.logvar_p.data = self.encoder2(z1_sample)

        optim_infer = torch.optim.Adam(phi_2.parameters(), lr=1e-2)
        inference_params = list(phi_2.parameters())

        grad_switch(inference_params, grad_bool=True)
        for infer_iter in range(self.n_iters):
            optim_infer.zero_grad()
            _, _, _, total_loss = self.iter_inference(x, phi_1=phi_1, phi_2=phi_2)
            optim_infer.step()
        grad_switch(inference_params, grad_bool=False)
    ...
    def forward_evaluate(self, x, x_clear=None):
        ...
        optim_infer = torch.optim.Adam(phi_1.parameters(), lr=1e-3)
        inference_params = list(phi_1.parameters())
        ...
        optim_infer = torch.optim.Adam(phi_2.parameters(), lr=1e-3)
        inference_params = list(phi_2.parameters())
    ...
    def iter_inference(self, x, phi_1=None, phi_2=None, mu=None, logvar=None, x_clear=None, evaluate=False):
        if phi_1 is not None:
            z1_sample, e_eps1 = self.r_sampling(phi_1.mu_p, phi_1.logvar_p)
            if phi_2 is not None:
                z2_sample, e_eps2 = self.r_sampling(phi_2.mu_p, phi_2.logvar_p)
                mu_d1, logvar_d1 = self.decoder2(z2_sample)
                z1_rec_sample, d_eps1 = self.r_sampling(mu_d1, logvar_d1)
                x_rec = self.decoder1(z1_sample)
                z2_var_params = (phi_2.mu_p, phi_2.logvar_p, e_eps2)
                z1_var_params = (phi_1.mu_p, phi_1.logvar_p, e_eps1)
...
def evaluate(model, data):
    ...
    phi_1, phi_2 = model(data)

    z1_sample, e_eps1 = model.r_sampling(phi_1.mu_p, phi_1.logvar_p)
    z2_sample, e_eps2 = model.r_sampling(phi_2.mu_p, phi_2.logvar_p)
...
def train_iHVAE(epoch):
    ...
    for idx, (X_train, y_train) in enumerate(train_loader):
        ...
        model.iter_inference(X_train, phi_1=None, phi_2=None, mu=mu1, logvar=logvar1)
        optimizer_iHVAE.step()
...
def reconstruct_images(model, data, x_clear=None):
    model.eval()
    phi_1, phi_2 = model.forward_evaluate(data, x_clear)
    z1_sample, _ = model.r_sampling(phi_1.mu_p, phi_1.logvar_p)
    x_rec = model.decoder1(z1_sample)
    return x_rec
...
z2 = 50
n_iters = 30
model = HiVAE(z1_dim=z1, z2_dim=z2, n_iters=n_iters).to(device)
...
PATH = '/home/lsinghal/new/HiVAE_type2_weights/HiVAE-type2-epochs-{}-z1-{}-z2-{}-train-{}-decoder-{}.pth'.format(epochs, z1, z2, n_iters, model.decoder_distribution)
